        ...
        <div className="p-4 space-y-0">
          {msgs.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-3 opacity-70 mt-10">
              ...
              <h3 className={cn("text-xl font-bold", THEME.text.header)}>Bem vindo a #general!</h3>
              <p className={cn("text-sm max-w-[200px]", THEME.text.secondary)}>
                Esse é o começo de uma conversa legendária.
                {isSingleUser && <span className="block mt-2 text-yellow-600 dark:text-yellow-400/80 text-xs">(Está apenas você agora, convide um amigo!)</span>}
              </p>
            </div>
          )}
      ...
      <AnimatePresence>
        {showScrollButton && (
          ...
          >
            {unreads > 0 ? (
              <>
                <span>{unreads} novas mensagens</span>
                <ArrowDown className="w-3 h-3" />
              </>
            ) : (
              <>
                <span>Suba para o presente</span>
                <ArrowDown className="w-3 h-3" />
              </>
            )}
          </motion.button>
